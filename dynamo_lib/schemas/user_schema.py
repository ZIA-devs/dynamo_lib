...
class UserSchema(BaseModel):
    ...
    company_id: int = Field(
        default=0, alias="company_id", description="Company ID associated with the user"
    )
    ...
    password: str = Field(
        default="", alias="password", description="Password of the user"
    )

    status: UserStatus = Field(
        default=UserStatus.ACTIVE, alias="status", description="Status of the user"
    )
