...
class BaseCrud(Generic[T]):
    ...
    def scan(cls, filter_expression: str | None = None) -> list[T]:
        items = dynamo_scan(
            table_name=cls.TABLE_NAME, filter_expression=filter_expression
        )
        if cls.debug:
            logger.info(f"Scanned items: {items}")
        return [cls.model(**item) for item in items if isinstance(item, dict)]

    @classmethod
    def list(cls, pk: str | int, sk: str | int = "") -> list[T]:
        items = dynamo_query(
            pk=str(pk), sk=cls.SK_MARKER + str(sk), table_name=cls.TABLE_NAME
        )
        if cls.debug:
            logger.info(f"Queried items for pk={pk}, sk={sk}: {items}")
    ...
    def get(cls, pk: str | int, sk: str | int = "") -> Optional[T]:
        ...
        if cls.debug:
            logger.info(f"Queried item for pk={pk}, sk={sk}: {result}")
    ...
    def add(cls, pk: str | int, sk: Optional[str | int], data: Dict[str, Any]) -> T:
        ...
        if cls.debug:
            logger.info(f"Adding item with pk={pk}, sk={sk_text}, data={data}")
        if not dynamo_create(
            pk=str(pk), sk=sk_text, item=data, table_name=cls.TABLE_NAME
        ):
    ...
    @classmethod
    def add_with_id(
        cls, pk: str | int, data: Dict[str, Any], id_key: str, sk: str | int = ""
    ) -> T:
        item_id = cls.get_new_id()
        data[id_key] = item_id
        sk = f"{sk}#{item_id}" if sk else str(item_id)
        if cls.debug:
            logger.info(f"Adding item with pk={pk}, sk={sk}, data={data}")
        if not dynamo_create(
            pk=str(pk), sk=cls.SK_MARKER + sk, item=data, table_name=cls.TABLE_NAME
        ):
            raise RuntimeError("Error adding item to DynamoDB")
        return cls.model(**data)
    ...
    def delete_starting_with(cls, pk: str | int, sk: str | int) -> bool:
        if cls.debug:
            logger.info(
                f"Deleting all items with pk={pk} starting with sk={cls.SK_MARKER + str(sk)}"
            )
    ...
    def delete(cls, pk: str | int, sk: str | int = "") -> bool:
        if cls.debug:
            logger.info(f"Deleting item with pk={pk}, sk={cls.SK_MARKER + str(sk)}")
        return dynamo_delete(
            pk=str(pk), sk=cls.SK_MARKER + str(sk), table_name=cls.TABLE_NAME
        )
    ...
    def update(cls, pk: str | int, sk: str | int = "", **kwargs) -> bool:
        ...
        for k, v in kwargs.items():
            if k not in allowed_fields:
                raise ValueError(f"Field '{k}' is not allowed for update.")
            update_values[allowed_fields[k]] = v.value if isinstance(v, Enum) else v

        if cls.debug:
            logger.info(
                f"Updating item with pk={pk}, sk={cls.SK_MARKER + str(sk)}, update_values={update_values}"
            )
        if not update_values:
            return False
        return dynamo_update(
            pk=str(pk),
            sk=cls.SK_MARKER + str(sk),
